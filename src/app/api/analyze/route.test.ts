...
const allowedQuota = { allowed: true, used: 1, remaining: 49, total: 50 };
...
describe('POST /api/analyze', () => {
    ...
    it('returns 429 BEFORE scraping when peek shows the user is already at the limit', async () => {
        mockAuth.mockResolvedValue(validSession);
        mockPeekRateLimit.mockResolvedValue({ allowed: false, used: 50, remaining: 0, total: 50 });
    ...
    it('returns 429 when peek allows but a concurrent request consumed the last slot', async () => {
        ...
        mockConsumeRateLimit.mockResolvedValue({ allowed: false, used: 50, remaining: 0, total: 50 });
    ...
    it('returns 200 with the analysis payload on the happy path', async () => {
        ...
        expect(body.rateLimit).toEqual({ remaining: 49, total: 50, limitReached: false });
